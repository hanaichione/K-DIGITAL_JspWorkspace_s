...
<input type="hidden" name="gCode" value=<%= cDTO.getgCode()%>>
<input type="hidden" name="gName" value=<%= cDTO.getgName()%>>
<input type="hidden" name="gPrice" value=<%= cDTO.getgPrice()%>>
<input type="hidden" name="gSize" value=<%= cDTO.getgSize()%>>
<input type="hidden" name="gColor" value=<%= cDTO.getgColor()%>>
<input type="hidden" name="gAmount" value=<%= cDTO.getgAmount()%>>
<input type="hidden" name="gImage" value=<%= cDTO.getgImage()%>>
<input type="hidden" name="orderNum" value=<%= cDTO.getNum()%>>	<!-- 주문번호 : cart에서 결제한 제품을 삭제할 때 이용 -->
	...
	<table width="80%" cellspacing="0" cellpadding="0">
		...
		<tr>
			<td>
				<table width="100%" cellspacing="0" cellpadding="0">
					...
					<tr>
						<td class="td_default" width="80"><%=cDTO.getNum()%></td>
						<td class="td_default" width="80"><img
							src="images/items/<%=cDTO.getgImage()%>.gif" border="0"
							align="center" width="80" /></td>
						<td class="td_default" width="300" style='padding-left: 30px'><%=cDTO.getgName()%>
							<br> <font size="2" color="#665b5f">[옵션 : 사이즈(<%=cDTO.getgSize()%>)
								, 색상(<%=cDTO.getgColor()%>)]
						</font></td>
						<td class="td_default" align="center" width="110"><%=cDTO.getgPrice()%>
						</td>
						<td class="td_default" align="center" width="90"><%=cDTO.getgAmount()%></td>
					...
					<tr>
						...
						<td class="td_default" align='right'><%=cDTO.getgAmount() * cDTO.getgPrice()%>원</td>
					</tr>
				</table>
		<tr>
			<td>
				...
				<table width="100%" cellspacing="0" cellpadding="0" border="1"
					...
					<tr>
						...
						<td height="35" class="td_default"><input
							...
							maxlength="15"
							value="<%=mDTO.getPhone1() + mDTO.getPhone2() + mDTO.getPhone3()%>"></input>
		...
		<tr>
			<td class="td_default"><input type="checkbox" name="same" id="same"> 배송지가 동일할 경우 선택하세요.</td>
		</tr>
		<!--  배송지 정보 시작-->
		<tr>
			<td height="30">
				...
				<table width="100%" cellspacing="0" cellpadding="0" border="1"
					...
					<tr>
						...
						<td height="35" class="td_default">
							<!-- 다음주소 시작--> <input type="text" name="post"
							id="sample4_postcode" placeholder="우편번호"> <input
							type="button" onclick="sample4_execDaumPostcode()"
							value="우편번호 찾기"><br> <input type="text" name="addr1"
